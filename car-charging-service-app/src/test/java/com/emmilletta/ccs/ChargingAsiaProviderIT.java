...
class ChargingAsiaProviderIT {
    ...
    @Test
    void startChargingWrongProviderUserId() throws Exception {
        UserEntity user = createUser();
        user.setProviderUserId("wrong_provider_user_id");
        userRepository.save(user);
        ...
        Assert.assertEquals(CcsResponseStatus.WRONG_PROVIDER_USER_ID, content.getResponseCode());
    ...
    void startChargingStationNotAvailable() throws Exception {
        UserEntity user = createUser();
        user.setProviderUserId("station_not_available");
        userRepository.save(user);
    ...
    void startChargingInternalServerError() throws Exception {
        UserEntity user = createUser();
        user.setProviderUserId("internal_server_error");
        userRepository.save(user);
    ...
    private UserEntity createUser() {
        UserEntity user = UserEntity.builder().userId("testUserId").userAccessKey("testUserAccessKey").providerUserId("testProviderUserId")
